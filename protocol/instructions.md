...
| Slot | AGENTS.md (cross-tool) | Claude Code | GitHub Copilot | Cursor |
|------|-----------------------|-------------|----------------|--------|
| `operational` | `AGENTS.md` | `CLAUDE.md` | `.github/copilot-instructions.md` | `.cursor/rules/` |
| `behavioral` | `AGENTS.md` (appended) | `AGENT.md` | Behavioral preference file (tool-defined) | Behavioral settings file (tool-defined) |
| `identity` | (not supported) | `SOUL.md` | Not supported | Not supported |

`AGENTS.md` is the cross-tool instruction file read natively by most implementations, and is the primary portable target for the `operational` slot (with `behavioral` content appended). Tool-specific files such as `CLAUDE.md` remain supported where an implementation reads them. A harness author writes their instructions once; a conformant implementation applies them to the right file(s) for the tool it is managing — typically `AGENTS.md` plus any tool-specific file.
...
|-------|------|----------|---------|-------------|
| `operational` | string or null | No | — | Instructions for how to work: build commands, architecture, gotchas, project conventions. Maps to `AGENTS.md` (the cross-tool standard), `CLAUDE.md` (Claude Code), `.github/copilot-instructions.md` (Copilot), `.cursor/rules/` (Cursor). |
| `behavioral` | string or null | No | — | Instructions for how to behave: communication tone, autonomy level, workflow conventions, what to ask vs. proceed silently. Maps to `AGENT.md` (Claude Code), an appended section of `AGENTS.md`, and behavioral preference files in other harnesses. |
...
<!-- End source: profile:data-engineer -->
```

Provenance markers allow users to audit which instruction blocks came from which harness in a composed session. They are HTML comments and are therefore invisible in rendered Markdown but visible in raw file inspection. Tools that do not parse HTML comments will display them as text; this is acceptable.
...
This meta-instruction is appended once after all merging is complete, regardless of inheritance depth. It is not subject to the `replace` mode — it cannot be overridden by a harness profile.
