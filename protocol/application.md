...
- The parser MUST NOT silently coerce types in ways that change semantics. In particular, the `version` field MUST be preserved as a string — `version: 1` (integer) is not equivalent to `version: "1"` (string).
...
| Enforce maximum inheritance depth | SHOULD | Resolve |
