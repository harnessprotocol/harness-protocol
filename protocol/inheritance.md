...
All skills from all parents are included in the effective skill set. If two harnesses declare a skill with the same `name`, the child's (or later parent's) declaration is used entirely. A child sets `enabled: false` to suppress a skill it inherits without re-declaring its `source`.

```
parent: pdf-forms@>=1.0.0, house-style
child:   pdf-forms (enabled: false)

effective: house-style (from parent); pdf-forms is suppressed by the child
```
...
If a parent declares no `permissions.tools.allow` list (the field is absent), it imposes no restriction — only parents that actively define an allow list participate in the intersection. If a parent declares `permissions.tools.allow` and a child does not, the child inherits the parent's full allow list unchanged. The intersection rule only applies when both parent and child declare an allow list.
...
**Merge behavior: accumulation as a ceiling — constraints only ever tighten.**

`policy` is the one section that does not follow child-wins or simple union/intersection. It is a *managed layer*: a constraint that downstream profiles may narrow but never widen. The effective policy is computed by accumulating every `policy` section in the resolution set so that the result is at least as strict as each contributor:

| Policy field | Accumulation rule |
|--------------|-------------------|
| `*.allowed-sources`, `permissions.tools.allow`, `permissions.network.allowed-hosts` | **Intersection** — a value must satisfy every layer that declares that allowlist/ceiling. |
| `*.denied-sources`, `permissions.tools.deny` | **Union** — any layer's denial applies. |
| `plugins.allowed-marketplaces` | **Intersection.** |
| `require-integrity` | **Monotonic OR** — `true` if any layer sets it `true`. |
...
| `skills` | Union by `name`; child/later wins (`enabled: false` suppresses) |
...
| `policy` | Accumulation as a ceiling — allowlists/ceilings intersect, denylists union, `require-integrity` monotonic; child can only tighten |
...
**Implementations MUST enforce a maximum inheritance depth of 5 levels.** A chain deeper than 5 levels MUST fail with a clear error:
...
A fixed depth limit ensures portability: a harness that validates under one implementation will not fail under another due to differing limits.
