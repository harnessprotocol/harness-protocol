...
See [Trust Boundaries](../security/trust-boundaries.md) for the full trust model. Key points relevant to source resolution:
...
- **Plugin integrity:** When a plugin entry includes `integrity.sha256`, implementations MUST verify the fetched `plugin.json` content against the declared hash. A mismatch is a fatal error. See [Integrity](../security/integrity.md) for the verification algorithm.
