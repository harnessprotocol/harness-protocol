...
MCP servers communicate over one of several transports. The Harness Protocol v1 supports:

| Transport | Value | Description |
|-----------|-------|-------------|
| `stdio` | `"stdio"` | Local process launched by the implementation; communicates via stdin/stdout. |
| `http` | `"http"` | Remote server accessed via HTTP(S) using the MCP Streamable HTTP transport. |
| `sse` | `"sse"` | Remote server accessed via HTTP(S) using the legacy MCP SSE transport. |
| `ws` | `"ws"` | Remote server accessed via WebSocket. |
...
The `http` transport connects to a remote MCP server using the MCP Streamable HTTP transport over HTTPS. Use this for MCP servers that run as network services rather than local processes.
