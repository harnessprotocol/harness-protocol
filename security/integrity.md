...
## Integrity for Skills and MCP Server Packages

Plugins are not the only executable third-party content a harness pulls in. Skills are behavioral instructions (and may bundle scripts) fetched from external sources, and `stdio` MCP servers run a package on the user's machine. Both are supply-chain surfaces with the same failure modes described above, and both now carry the same optional `integrity.sha256` field:
...
The same format, scope, and verification rules apply: a 64-character lowercase hex SHA-256 over the fetched artifact, verified before use, fatal on mismatch. For skills, the hash covers the skill archive (the `SKILL.md` directory as distributed). For MCP servers, `integrity` is available on `stdio` declarations, where there is a fetchable package; remote transports have no archive to hash and rely on TLS and registry/identity verification instead (see [MCP Declarations](mcp-declarations.md#server-provenance-and-integrity)).

This closes a gap that earlier versions of this document called out: plugin integrity never covered the MCP server binaries a `stdio` declaration invokes, nor skill content. Per-artifact `integrity` now spans all three.

The risk this addresses is not hypothetical. Public skill and MCP marketplaces have shipped capability artifacts containing credential exfiltration, untrusted runtime fetches, and malicious payloads. A pinned hash means a harness an author reviewed cannot be silently swapped for a tampered artifact after publication. The transparency over *what* a skill instructs the agent to do is handled separately — see [Skill Behavioral Injection](skill-injection.md).
