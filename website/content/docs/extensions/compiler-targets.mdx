...
`.agents/skills/` is the agentskills.io-standard project-local shared location, supported by both Cursor and Copilot. When compiling for either of those targets, the compiler defaults to `.agents/skills/<name>/` unless the harness explicitly declares a tool-specific override.
