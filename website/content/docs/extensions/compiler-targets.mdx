...
description: "v2 design sketch: compiler target declarations for cross-tool configuration."
---
...
`.agents/skills/` is the conventional project-local shared skills location, supported by both Cursor and Copilot. When compiling for either of those targets, the compiler defaults to `.agents/skills/<name>/` unless the harness explicitly declares a tool-specific override.
