---
title: Terminology
description: Glossary of all terms used in the Harness Protocol specification.
---
...
In `harness.yaml`, plugins are declared in the `plugins[]` array. Each entry references a plugin by `source` (owner/repo) and `version`. The plugin itself declares its capabilities in a `plugin.json` manifest. See [Plugin Manifest](../specification/plugin-manifest).
...
Skills reach a harness two ways: declared directly in the top-level `skills[]` array (see HEP-4 and [Profile Schema: skills](../specification/profile-schema#skills)), or bundled by a plugin via its `plugin.json` manifest. A directly-declared skill does not require authoring a plugin.
...
See [Application Semantics](../specification/application) for the full specification.
...
See `CONTRIBUTING.md` in the repository root for the full HEP submission process.
