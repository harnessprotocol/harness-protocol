---
title: Architecture
description: System diagram, layer interactions, and trust model.
---
...
The Schema layer defines how a `harness.yaml` is structured, validated, and applied. At a high level, the flow has three phases (for the complete 6-step pipeline, see [Application Semantics](../specification/application)):
...
1. Resolve `extends` — fetch and validate parent harnesses, then merge fields according to section-specific inheritance rules (see [Profile Schema: extends](../specification/profile-schema#extends)).
