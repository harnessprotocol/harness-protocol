...
Instructions do not follow the simple union/intersection model. Instead, the child's `import-mode` field controls how its instructions relate to the cumulative parent instructions. See [Instructions: Import Modes](./instructions#import-modes) for the full specification.
...
The accumulated policy is then enforced against the merged effective configuration during [Application Step 5: Enforce Policy](./application#step-5-enforce-policy). A violation is a fatal error; the harness is not applied. A resolution set with no `policy` section imposes no constraints — the behavior before this section existed.
