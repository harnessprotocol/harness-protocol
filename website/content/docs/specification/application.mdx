...
description: The application pipeline, effective configuration contract, error categories, and atomicity guarantees.
---
...
This document covers the abstract application semantics. It does not mandate a specific implementation strategy (compilation to config files vs. runtime configuration). Both strategies produce the same normative output: the effective configuration. For the non-normative compilation approach, see [Compiler Targets](../extensions/compiler-targets).
...
1. **Resolve `extends` chain** — Fetch and validate each parent harness, recursively. Detect circular dependencies (fatal). Enforce depth limits (see [Inheritance](./inheritance)). Local paths (`./`, `../`) are resolved relative to the directory containing the consuming harness file.
...
For the complete source resolution algorithm, see [Source Resolution](./source-resolution).
...
For the complete merge rule specification, see [Inheritance](./inheritance).
...
A policy violation MUST be surfaced as a clear error. Implementations MUST NOT silently strip violating entries and apply a degraded configuration. A document with no `policy` in its resolution set skips this step entirely (the v1 behavior before this step existed). See [Profile Schema: policy](./profile-schema#policy) and [Inheritance](./inheritance).
...
**Compilation.** The implementation generates tool-specific configuration files (e.g., `.mcp.json`, `CLAUDE.md`, `settings.json` for Claude Code). The generated files are the effective configuration materialized as files. This is a non-normative strategy; see [Compiler Targets](../extensions/compiler-targets) for one example.
