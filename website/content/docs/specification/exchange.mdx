---
title: Exchange
description: "(v2 draft) The signed offer envelope and consent-first peer-to-peer fragment sharing flow."
---

This document specifies the **Exchange layer** of the Harness Protocol: a push-based, consent-required flow for sharing a single `kind: fragment` document from one person to another. Exchange is introduced in the v2 milestone and is specified normatively by [HEP-7](https://github.com/harnessprotocol/harness-protocol/blob/main/heps/hep-0007-exchange-layer.md). The offer envelope is validated by [`exchange.schema.json`](https://github.com/harnessprotocol/harness-protocol/blob/main/schema/draft/exchange.schema.json).
...
Exchange is strictly **one-to-one (push)**. One-to-many distribution — publish once, many fetch — is the [Registry](/docs/specification/registry) layer's responsibility, not Exchange's.
...
An accepted fragment is applied via the same inheritance mechanism as any other `extends` entry (see [Inheritance](/docs/specification/inheritance)) — it gains no special privilege from having arrived through Exchange. The receiver's harness records provenance:
...
**There is no protocol-level key revocation in v2.** A compromised key is rotated and the new key communicated out of band; offers from a known-compromised fingerprint can be rejected manually. Time-stamped key rotation with a signed revocation log is v3 [Registry](/docs/specification/registry) scope.
...
For how Exchange's ed25519 transit identity relates to the Registry/plugin minisign provenance track, see the [crypto map](/docs/security/crypto-map). Exchange's threat model is specified in [Exchange security](/docs/security/exchange).
