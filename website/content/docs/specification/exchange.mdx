...
An accepted fragment is applied via the same inheritance mechanism as any other `extends` entry (see [Inheritance](/docs/specification/inheritance)) — it gains no special privilege from having arrived through Exchange. The receiver writes the fragment into a local exchange store and references it with a standard v1 **local source** (a `./` relative path; see [Source Resolution](/docs/specification/source-resolution)). No new `harness.yaml` fields and no new source schemes are introduced:
