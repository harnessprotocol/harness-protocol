...
The `sse` (Server-Sent Events) and `ws` (WebSocket) transports share the same field structure as `http` (`transport`, `url`, `headers`). Use `sse` for legacy MCP SSE servers; use `ws` for WebSocket-based servers. See [MCP Declarations](./mcp-declarations) for full details and security considerations for all network transports.
