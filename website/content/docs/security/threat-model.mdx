---
title: Threat Model
description: Threat model and security design for the Harness Protocol.
---
...
- The `source` field pattern `^[a-zA-Z0-9_][a-zA-Z0-9_.-]*/[a-zA-Z0-9_][a-zA-Z0-9_.-]*$` prevents path traversal in the source value itself, but does not prevent typosquatting. Users must visually verify source identifiers.
