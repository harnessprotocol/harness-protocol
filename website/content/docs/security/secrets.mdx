---
title: Secrets
description: Sensitive variable handling, secrets patterns, and the when field.
---

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="warn" title="Never store secret values in harness.yaml">
  The `env` block declares which variables a harness needs — it never stores their values. Profile authors MUST NOT write secret values into `harness.yaml`. Implementations MUST reject profiles that contain plaintext credentials in any field.
</Callout>
