---
title: Integrity
description: Content integrity verification via SHA-256 checksums.
---

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="warn" title="Integrity verification is mandatory">
  Conforming implementations MUST verify `integrity.sha256` when the field is present and MUST fail installation if the computed hash does not match. Skipping verification silently defeats the entire integrity model.
</Callout>
...
> **Scope note.** "v2" here labels a *supply-chain hardening* track — making `integrity.sha256` required and adding publisher signing. It is distinct from the **Exchange** and **Registry** layers ([HEP-7](https://github.com/harnessprotocol/harness-protocol/blob/main/heps/hep-0007-exchange-layer.md), [HEP-8](https://github.com/harnessprotocol/harness-protocol/blob/main/heps/hep-0008-registry-layer.md)) that open the v2 milestone and add no `harness.yaml` fields. Promoting integrity to required and adding minisign signing are normative changes that require their own HEP. The Registry's transparency log (HEP-8) records hash-based `index`/`delist` events in v2; the per-release minisign signatures described below are added to those log entries when publisher signing ships. See the [crypto map](/docs/security/crypto-map) and [Registry](/docs/specification/registry).
