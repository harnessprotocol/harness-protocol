---
title: Skill Behavioral Injection
description: Skill files as a vector for persistent behavioral modification — marketing directives, persona changes, and out-of-scope data observation.
---

import { Callout } from 'fumadocs-ui/components/callout';

<Callout type="warn" title="Active threat">
  Skill behavioral injection has been observed in the wild (March 2026). A commercial plugin embedded a persistent sales directive in its skill file that modified the AI's recommendations for the full session. The primary defense is displaying skill content to the user before installation.
</Callout>
...
Unlike harness profile instruction injection (covered in [Instruction Injection](/docs/security/instruction-injection)), skill injection is not primarily about overriding the user's safety rules. The threat is subtler: a skill file that appears to define a legitimate workflow but also contains persistent behavioral directives that serve the plugin author's interests at the user's expense.
...
If yes: likely legitimate skill content. If no, or if it asks the AI to do something outside the scope of the skill's function: treat as behavioral injection.
...
A skill declared directly in the top-level `skills` section (see [Profile Schema: skills](../specification/profile-schema#skills)) carries optional `source`, `version`, and `integrity.sha256` fields. These do not detect behavioral injection — a hash verifies that the skill content is unchanged, not that the content is benign — but they bound the threat in two ways:
...
- **Mandatory verification.** An organization can set `policy.require-integrity: true` to reject any skill without a verifiable hash, and `policy.skills.allowed-sources` / `denied-sources` to restrict which sources a skill may come from at all. See [Integrity](integrity#mandatory-integrity-via-policy) and HEP-6 (team governance).
