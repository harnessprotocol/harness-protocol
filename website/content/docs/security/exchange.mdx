...
A fragment is a legitimate vector for behavior change: it can declare instructions, skills, MCP servers, and permissions. A malicious sender could craft a fragment whose instructions attempt to subvert the receiver's agent (see [Instruction Injection](/docs/security/instruction-injection) and [Skill Injection](/docs/security/skill-injection)).
