...
description: Trust boundaries between spec, implementations, profiles, and remote content.
---
...
The spec does not execute anything. It cannot be "attacked" directly — only bypassed by non-conforming implementations or by spec design flaws. Spec vulnerabilities (schema constraints that can be bypassed, underspecified security behaviors) should be reported to the spec maintainers. See [SECURITY.md](https://github.com/harnessprotocol/harness-protocol/blob/main/SECURITY.md).
